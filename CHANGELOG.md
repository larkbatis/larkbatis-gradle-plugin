...
- **Expose 5 processor options directly in the `larkbatis` block**:
  `mapUnderscoreToCamelCase`, `typeHandlers`, `registryPackage`, `springConfig`,
  and `springConfigPackage` can now be configured directly instead of passing
  `-A` compiler arguments manually. The plugin will skip any unset options,
  letting the processor apply its own defaults.
